...
## Phase 1 — Environment Setup (Isaac Sim Foundation)

### Objectives
- Set up Isaac Sim (Omniverse) and configure reproducible simulation environments.
- Prepare procedural scenes and ROS 2 integration.

### Key Actions
1. Install Isaac Sim (2023.1+) at `/opt/NVIDIA/isaac_sim` on `sim-01`.
2. Create workspace `~/rapid_v2_ws` with repositories:
   - `rapid-sim` — extensions, scenes, and bridge utilities.
   - `rapid-planner` — kinodynamic planner and trajectory generator.
   - `rapid-datalog` — episode logging and shard packer.
   - `rapid-tools` — CLI runner, testing utilities, job orchestration.
3. Configure scene families (Office, Warehouse, Forest, Urban, Cave, Maze, Mine, Shipyard, Ruins, Jungle).
4. Set up Isaac Replicator randomization for lighting, materials, and obstacles.
5. Verify ROS 2 bridge topics: `/depth`, `/odom`, `/imu`.
6. Configure sensors (stereo depth, 20 Hz) and logging at `/data/rapid_v2/runtime/`.

### Deliverables
- Isaac Sim functional and reproducible.
- Scene seeds stored under `/data/rapid_v2/runtime/scenes.jsonl`.
- Sensor configuration YAMLs committed to version control.
...
### Deliverables
- `/data/rapid_v2/dataset/shards/{easy,medium,hard}/` populated.
...
### Deliverables
- Final dataset under `/data/rapid_v2/dataset/`.
...
|:------|:------------------|
| 1 | Isaac Sim fully functional; reproducible scenes |
...
## Final Outcome
- Full Isaac Sim–based expert data collection pipeline.
